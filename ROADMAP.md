...
**Couche de lecture — la bibliothèque de lentilles.** Le schéma vivant se lit sous plusieurs angles : la **bibliothèque de lentilles** (`lugia_bibliotheque_lentilles.md`) définit 22 angles de lecture du même jumeau, combinables avec les 5 niveaux de zoom. Familles : fondamentales (santé, conformité, données, maturité, charge/temps, coût/valeur, fragilité, documentation), stratégiques (alignement ⭐, ROI, effort de changement), métier (automatisation ⭐, adoption, dépendance externe), temporelles (évolution, récence, saisonnalité), humaines (charge mentale, satisfaction, compétence), protection (cybersécurité, réversibilité). Cinq règles de conception (R1 unicité de question → R5 légende contextuelle), un ciblage par public (levier produit : lentilles avancées réservées aux offres supérieures) et trois vagues de développement (alignement stratégique + automatisation/manuel d'abord).

**Nouvelle dimension — Circulation du savoir** (`lugia_circulation_savoir.md`) : la diffusion des bonnes pratiques entre participants, distincte de la documentation (existence du savoir) et du processus (façon de faire). Se traduit dans le moteur WSF par un nouveau type de liaison `TRANSMET` (un participant transmet un savoir à un autre), une lentille dédiée (vert = diffusé, ambre = partiel/informel, rouge = en silo) et une échelle de diffusion 0-5 (individuelle → identifiée → formalisée → partagée → adoptée → améliorée collectivement). Recoupe la lentille Fragilité sous l'angle « opportunité manquée » plutôt que « risque ». Valeur maximale en cabinet de groupe / MSP. Chantiers associés : identifier les pépites, briser un silo, capitaliser une bonne pratique, onboarding par les bonnes pratiques.
