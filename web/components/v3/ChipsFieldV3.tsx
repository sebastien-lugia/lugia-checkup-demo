...
export type V3Field = {
  ...
  /** Force une grille à N colonnes (ex. 2 pour layout 2x2). Sinon flex-wrap naturel. */
  gridCols?: 2 | 3;
  /** Insère un saut de ligne après le Nème chip (flex-wrap). Permet d'imposer un wrap sans étirer. */
  wrapAfter?: number;
};
...
export function ChipsFieldV3({
  ...
  return (
    <section style={{ marginBottom: 36 }}>
      ...
      {(() => {
        // Helper : rendu d'une chip option
        const renderChip = (opt: V3ChipOption) => {
        ...
        // Helper : freeAdds + bouton "+ Ajouter" (rendus en dernier visuel)
        ...
        const flexStyle = { display: "flex", flexWrap: "wrap", gap: 8 } as const;

        // Layout 1 : wrapAfter défini → deux conteneurs flex stackés (gaps natifs uniformes)
        if (field.wrapAfter !== undefined) {
          const firstHalf = field.options.slice(0, field.wrapAfter);
          const secondHalf = field.options.slice(field.wrapAfter);
          return (
            <div
              role={isMulti ? "group" : "radiogroup"}
              aria-label={field.label}
            >
              <div style={{ ...flexStyle, marginBottom: 8 }}>
        ...
        // Layout 2 : gridCols défini → CSS Grid
        if (field.gridCols) {
          return (
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `repeat(${field.gridCols}, auto)`,
                gap: 8,
              ...
              aria-label={field.label}
            >
              {field.options.map(renderChip)}
              {trailingChildren}
            </div>
          );
        }

        // Layout 3 (défaut) : flex-wrap naturel
        return (
          <div
            style={flexStyle}
            role={isMulti ? "group" : "radiogroup"}
