  ...
  return (
    ...
    >
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "0 24px" }}>
        {/* Bouton retour discret — pattern aligné sur ListChantiersV3 */}
        {onBack && (
          <button
            type="button"
            onClick={onBack}
        ...
        {/* ── Header ──
            Eyebrow petit trait à gauche (variante intro / parcours).
            Les autres eyebrows de la page conservent le filet à droite. */}
        <p
          style={{
            fontFamily: fonts.mono,
            fontSize: 10,
        ...
        {/* ── Section motivation de départ — même pattern que PhraseChoc :
            eyebrow mono caps avec filet à droite + phrase Lora discrète. */}
        <div style={{ margin: "0 0 48px" }}>
          ...
          >
            <span style={{ flexShrink: 0 }}>Votre motivation de départ</span>
            <span
              aria-hidden="true"
        ...
        <section style={{ marginBottom: 48 }}>
          ...
          >
            Plus chaque axe s&apos;éloigne du centre, plus l&apos;organisation est mature sur cet aspect.
          </p>
...
function RiskBadge({ theme, level }: { theme: V3Theme; level: RiskLevel }) {
  const palette = paletteFor(theme);
  const colors = {
    // crit = signal-warn pleine intensité (urgence opérationnelle)
    crit: { txt: palette.signalWarn.default, bg: palette.signalWarn.surface, border: palette.signalWarn.border },
    // warn = signal-warn atténué (vigilance, pas urgence)
    //   → bg et border en alpha réduit, texte légèrement opacifié
    //   pour créer un écart visuel net avec crit sans changer de palette.
    warn: { txt: `${palette.signalWarn.default}B0`, bg: `${palette.signalWarn.default}10`, border: `${palette.signalWarn.default}40` },
    // opt = argent neutre (optimisation, ton plus doux)
    ...
    <div style={{ margin: "0 0 48px" }}>
      ...
      >
        ...
        >
          ...
          >
            <span aria-hidden="true" style={{ width: 14, height: 1, background: palette.signalWarn.default, flexShrink: 0 }} />
            Ce qui fragilise
          </p>
          {risques.map((item, i) => (
            <div
      ...
      >
        ...
        >
          ...
          {/* Vignette pill du niveau de maturité — pas de toggle « + », l'affordance
              de dépli reste portée par la phrase hint « ↓ Cliquer pour Forces et pistes d'actions ». */}
          <span
            style={{
              display: "inline-flex",
              alignItems: "center",
        ...
        {!open && (
          ...
          >
            ↓ Cliquer pour Forces et pistes d&apos;actions
          </p>
        )}
      </button>
        ...
        {opp.recommended && (
          <span
            style={{
              ...
              gap: 5,
              padding: "2px 9px",
              background: `${palette.argent}14`,
              border: `1px solid ${palette.argent}66`,
...
function TagTemporel({ theme, effort }: { theme: V3Theme; effort: 1 | 2 | 3 }) {
  const palette = paletteFor(theme);
  // Mapping charte (composant 13 plan d'action) :
  //   effort 1 → QUICK   — accent navy (= ivoire en Nuit, navy en Jour)
  //   effort 2 → MEDIUM  — argent (matière neutre)
  //   effort 3 → INVEST  — warn (or-signal — investissement signalé)
  const tag =
    effort === 1
...
function RadarSvg({
  ...
  return (
    ...
    >
      ...
      {/* Labels d'axes aux sommets — alignés sur le radar live du parcours
          (CLIENTS / ÉQUIPE / OUTILS, mono caps, posés juste à l'extérieur des
          vertices avec un textAnchor qui dépend de l'orientation). */}
      {(() => {
        ...
        const ANCHOR: Record<AxisId, "start" | "middle" | "end"> = {
          A: "end",   // vertex à gauche → label à gauche du sommet
          B: "start", // vertex en haut-droite → label à droite
          C: "start", // vertex en bas-droite → label à droite
        };
        // Le label A (vertex à gauche) tombe sur la ligne horizontale
        // d'annotation BC à y=CY → décalage vertical -18px pour le clearance.
        const Y_OFFSET: Record<AxisId, number> = { A: -18, B: 0, C: 0 };
        const labelOffset = 24;
        return AXES.map((ax) => {
          const [lx, ly] = pt(ax.angle, R + labelOffset);
          return (
            <text
              key={`axislabel-${ax.id}`}
              x={lx}
              y={ly + Y_OFFSET[ax.id]}
              textAnchor={ANCHOR[ax.id]}
      ...
      {annotations.map((ann, idx) => {
        ...
        // Charte A1 : crit utilise signalWarn (signal croisé canonique),
        // les autres annotations aussi → un seul ton, le warn.
        const badgeColor = palette.signalWarn.default;
      ...
      {/* Dots avec glow (charte A1 : pas de trou central — point plein argent) */}
      {AXES.map((ax, i) => (
        <circle
          key={ax.id}
          cx={dp[ax.id][0]}
