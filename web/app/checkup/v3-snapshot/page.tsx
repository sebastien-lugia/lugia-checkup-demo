...
import { paletteFor, type V3Theme, levelOf, LEVELS } from "@/lib/v3-snapshot/tokens";
import { type V3Step, nextStep, prevStep } from "@/lib/v3-snapshot/state";
...
import { Topbar } from "@/components/v3-snapshot/Topbar";
import { ThemeToggleV3 } from "@/components/v3-snapshot/ThemeToggleV3";
import {
  ENERGY_FIELD,
  IntroV3,
  ProfilStep1V3,
  ProfilStep2V3,
  EnergyV3,
} from "@/components/v3-snapshot/screens";
import { BlocV3, BlockTransitionV3 } from "@/components/v3-snapshot/screens_blocs";
import { RadarLiveV3 } from "@/components/v3-snapshot/RadarLiveV3";
import { ResultatsV3, type V3Opp } from "@/components/v3-snapshot/ResultatsV3";
import { ModuleV3 } from "@/components/v3-snapshot/ModuleV3";
import { getModule } from "@/lib/v3-snapshot/modules_data";
import {
  pickOppsFromScores,
  getOpp,
} from "@/lib/v3-snapshot/opps_catalog";
import { ListChantiersV3 } from "@/components/v3-snapshot/ListChantiersV3";
import { openCalendly } from "@/lib/v3-snapshot/links";
...
function buildLocalScores(answers: Record<string, string>): LocalScores {
  ...
  for (const blockId of blocks) {
    ...
    const answered = bloc.questions.filter((q) => answers[q.id]);
    out.completeness[blockId] = answered.length / bloc.questions.length;
...
function CheckupV3BrandPageContent() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        // 5. Si on vient avec ?view=results, basculer directement sur les résultats
  ...
  const localScores = useMemo(() => buildLocalScores(answers), [answers]);
  ...
  if (step === "resultats") {
    ...
    // Phrase choc : début + strong en warn
    const phraseChocBefore =
      "Paradoxe : bon parcours patient, organisation fragile. Vous portez seul ce que l'équipe et les outils devraient porter —";
    const phraseChocAfter = "c'est ce déséquilibre qui explique la charge que vous ressentez en fin de semaine.";

    // Bilan global — exemples basés sur les scores actuels (pour la démo)
    const bilanForces = [
      { title: "Parcours patient structuré", desc: "Tri des urgences en place, suivi des chroniques actif." },
      { title: "Équipe présente", desc: "Les personnes sont là — ce qui manque, c'est le cadre pour les rendre autonomes." },
      { title: "Logiciel connu", desc: "Quelques fonctions maîtrisées — la base est là pour aller plus loin." },
    ];
    const bilanRisques = [
      { title: "Dépendance à votre présence", desc: "Toute absence déstabilise. L'organisation n'existe pas sans vous.", risk: "crit" as const },
      { title: "Outils sous-exploités", desc: "Le logiciel porte moins de 30 % de son potentiel.", risk: "warn" as const },
      { title: "Patients silencieux", desc: "Pas de système pour repérer les chroniques qui n'appellent plus.", risk: "warn" as const },
    ];

    const radarAnnotations = [
      { axis: "B" as const, side: "right" as const, title: "Rôles sans cadre formel.", sub: "Absences déstabilisantes.", badge: "crit" as const },
      { axis: "C" as const, side: "right" as const, title: "Logiciel sous-exploité.", sub: "Pilotage absent.", badge: "warn" as const },
      { axis: "BC" as const, side: "left" as const, title: "Manque de cadre.", sub: "Charge ressentie amplifiée.", badge: "crit" as const },
    ];
    ...
    return (
      <>
        ...
        <ResultatsV3
          ...
          axisADetail={{
            summary: "Un parcours structuré sur les situations courantes — les marges concernent les silences (patients perdus de vue, alertes résultats).",
            forces: ["Tri des urgences du jour formalisé.", "Suivi des chroniques actif avec rappels."],
            marges: [
          ...
          axisBDetail={{
            summary: "Les rôles existent mais sans cadre formel. Les angles morts apparaissent dès qu'une absence se présente.",
            forces: ["L'équipe est présente et engagée — le levier existe."],
            marges: [
          ...
          axisCDetail={{
            summary: "Quelques fonctions maîtrisées, le reste en sommeil. Le logiciel pourrait porter bien plus qu'aujourd'hui.",
            forces: ["Base logiciel présente — formation courte suffit."],
            marges: [
          ...
          onOpenModule={(id) => {
            ...
            if (id.startsWith("__")) {
              alert("Action " + id + " — à brancher en T-V3-11.");
              return;
            }
            setOpenModuleId(id);
          }}
          onOpenAll={() => setListChantiersOpen(true)}
          onAutonomie={() => alert("Discussion autonome avec assistant Lugia — à brancher en T-V3-11.")}
          onLugia={() =>
    ...
    <>
      ...
      {step === "transition_C" && (
        <BlockTransitionV3
          closedBloc="C"
          scores={numScoresOrNull}
          nextBlocLabel="Votre profil et trois leviers à votre main"
          onContinue={() => {
            setStep("resultats");
 ...
 * useSearchParams() pour éviter "missing-suspense-with-csr-bailout".
 */
